const finhub_basepath = "https://finnhub.io/api/v1";
const finhub_apiKey = import.meta.env.VITE_API_kEY_FINHUB;

const twelveData_basepath = "https://api.twelvedata.com";
const twelve_apikey = import.meta.env.VITE_API_KEY_TWELVE;
...
export const searchSymbol = async (query) => {
  const url = `${finhub_basepath}/search?q=${query}&token=${finhub_apiKey}`;
 ...
 * @returns {Promise<Object[]>} Response array of best stock matches
 */
export const stockSymbol = async () => {
  const url = `${finhub_basepath}/stock/symbol?exchange=US&token=${finhub_apiKey}`;
  const response = await fetch(url);
...
export const fetchStockDetails = async (stockSymbol) => {
  const url = `${finhub_basepath}/stock/profile2?symbol=${stockSymbol}&token=${finhub_apiKey}`;
  const response = await fetch(url);
...
export const fetchQuote = async (stockSymbol) => {
  const url = `${finhub_basepath}/quote?symbol=${stockSymbol}&token=${finhub_apiKey}`;
  const response = await fetch(url);
...
export const fetchHistoricalData = async (
  ...
  const url = `${finhub_basepath}/stock/candle?symbol=${stockSymbol}&resolution=${resolution}&from=${from}&to=${to}&token=${finhub_apiKey}`;
  const response = await fetch(url);
